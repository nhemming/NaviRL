...
class DestinationSensor(Sensor):
    ...
    def __init__(self, id, max_dst, name, owner, target):
        super(DestinationSensor,self).__init__(id, name, owner)
        self.target = target
        self.state_dict['angle'] = 0.0  # [rad]
        self.state_dict['distance'] = 0.0  # [m]
        self.state_dict['distance_complement'] = 0.0  # [m]
        self.state_dict['actual_distance'] = 0.0  # [m]
        self.state_dict['max_dst'] = max_dst
    ...
    def update(self, time, entities, sensors):
        ...
        self.state_dict['actual_distance'] = np.sqrt(sep_vec[0]*sep_vec[0] + sep_vec[1]*sep_vec[1])
        if self.state_dict['actual_distance'] > self.state_dict['max_dst']:
            self.state_dict['distance'] = self.state_dict['max_dst']
        else:
            self.state_dict['distance'] = self.state_dict['actual_distance']

        self.state_dict['distance_complement'] = self.state_dict['max_dst']-self.state_dict['distance']
